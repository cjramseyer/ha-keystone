...
const secretsDirectory =
  process.env.SECRETS_DIR || path.join(dataDirectory, ".secrets");
...
function normalizeProfile(profile) {
  if (
    !Array.isArray(profile.license_options) ||
    !profile.license_options.length
  )
    profile.license_options = defaultLicenseOptions(profile.trial_days || 30);
...
const profiles = {
  "home-assistant-companion": {
    id: "app_profile_home_assistant_companion_v1",
    app_id: "home-assistant-companion",
    name: "HA Companion",
    installation_limit: 2,
    token_audience: "home-assistant-companion",
    features: [
      "remote_access",
    ...
    plans: ["base", "pro"],
    license_options: defaultLicenseOptions(30),
  },
  "home-assistant-voice": {
    id: "app_profile_home_assistant_voice_v1",
    app_id: "home-assistant-voice",
    name: "HA Voice",
    installation_limit: 3,
    token_audience: "home-assistant-voice",
...
function ensureRuntimeFiles() {
  ...
  if (!fs.existsSync(dataFile)) {
    writeData({
      ...
      activations: [],
      audit_events: [],
      app_profiles: Object.values(profiles),
    });
  } else {
    const data = readData();
    ...
    data.app_profiles.forEach((profile) => {
...
function publicKeyFromInput(value) {
  ...
  if (typeof encoded !== "string" || !encoded.trim())
    throw new Error("Activation request is missing its public key");
  if (
    typeof value === "object" &&
    value.algorithm &&
    value.algorithm !== "Ed25519"
  )
    throw new Error("Activation public key algorithm must be Ed25519");
  if (
    typeof value === "object" &&
    value.encoding &&
    value.encoding !== "base64url"
  )
    throw new Error("Activation public key encoding must be base64url");
...
function activationMessage(request) {
  return `${request.app_id}.${request.instance_id}.${request.instance_key_id}.${request.nonce}`;
}
function verifyActivationRequest(request) {
  ...
  if (
    ...
    throw new Error(
      "Activation request is missing required instance proof fields",
    );
  const publicKey = publicKeyFromInput(request.instance_public_key);
  const signatureValue =
    typeof request.signature === "object" && request.signature !== null
      ? request.signature.value
      : request.signature;
  if (typeof signatureValue !== "string" || !signatureValue.trim())
    throw new Error("Activation request is missing its signature");
  if (
    typeof request.signature === "object" &&
    request.signature.algorithm &&
    request.signature.algorithm !== "Ed25519"
  )
    throw new Error("Activation signature algorithm must be Ed25519");
  const valid = crypto.verify(
    null,
    Buffer.from(activationMessage(request)),
    publicKey,
...
function signLicense(payload) {
  ...
  const signature = crypto
    .sign(null, Buffer.from(signingInput), fs.readFileSync(privateKeyFile))
    .toString("base64url");
...
function verifyLicenseToken(token) {
  ...
  if (
    payload.aud !== profiles[payload.app_id]?.token_audience ||
    Date.parse(payload.expires_at) <= Date.now()
  )
    throw new Error("License audience or expiration is invalid");
  const storedLicense = readData().licenses.find(
    (license) => license.license_id === payload.license_id,
  );
  if (storedLicense?.status === "revoked")
...
function safeLicense(license) {
  const { token, license_payload, activation_request, ...metadata } = license;
  return metadata;
}
function legacyLicensePayload(license) {
  return {
    ...
    aud: license.aud,
    instance_binding: license.instance_binding,
    option_id: license.option_id,
    option_type: license.option_type,
    option_length_days: license.option_length_days,
    plan: license.plan,
...
function storedLicensePayload(license) {
  return license.license_payload
    ? JSON.parse(JSON.stringify(license.license_payload))
    : legacyLicensePayload(license);
}
function calculateMetrics(data) {
  const now = Date.now();
  const validLicenses = data.licenses.filter(
    (license) =>
      license.status !== "revoked" && Date.parse(license.expires_at) > now,
  );
  const paidLicenses = data.licenses.filter(
    (license) =>
      license.license_type === "paid" || license.option_type === "pro",
  );
  const renewedLicenseIds = new Set(
    data.audit_events
      .filter((event) => event.action === "license.renewed")
      .map((event) => event.license_id),
  );
  return {
    active_licenses: validLicenses.length,
    trials_in_progress: validLicenses.filter(
      (license) =>
        license.license_type === "trial" || license.option_type === "trial",
    ).length,
    renewal_rate: paidLicenses.length
      ? Number(
          (
            (paidLicenses.filter((license) =>
              renewedLicenseIds.has(license.license_id),
            ).length /
              paidLicenses.length) *
            100
          ).toFixed(1),
...
async function handleApi(request, response) {
  ...
  if (request.method === "POST" && pathname === "/api/auth/login") {
    ...
    if (
      ...
      !verifyPassword(body.password, storedAuth.password_hash)
    )
      return sendError(response, 401, "Invalid administrator credentials");
    ...
    return sendJson(
      ...
      { authenticated: true },
      {
        "Set-Cookie": `${sessionCookie}=${encodeURIComponent(session)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800`,
      },
    );
  }
  ...
  if (request.method === "GET" && pathname === "/api/auth/session") {
    ...
    return sendJson(response, 200, {
      ...
      profile: authenticated
        ? { username: auth.username, name: auth.name, email: auth.email }
        : null,
    });
  }
  ...
  if (request.method === "PUT" && pathname === "/api/auth/profile") {
    ...
    writeData(data);
    return sendJson(response, 200, {
      profile: { username: auth.username, name: auth.name, email: auth.email },
    });
  }
  if (request.method === "GET" && pathname === "/api/state") {
    ...
    return sendJson(response, 200, {
      ...
      licenses: data.licenses.map(safeLicense),
      audit_events: data.audit_events.slice(-20).reverse(),
  ...
  if (request.method === "POST" && licenseRenewMatch) {
    ...
    const license = data.licenses.find(
      (record) => record.license_id === licenseId,
    );
    if (!license) return sendError(response, 404, "License not found");
    if (license.status === "revoked")
      return sendError(response, 409, "Revoked licenses cannot be renewed");
    ...
    const renewedUntil = new Date(
      currentExpiry + extensionDays * 86400000,
    ).toISOString();
    license.expires_at = renewedUntil;
    if (license.license_payload)
      license.license_payload.expires_at = renewedUntil;
    ...
    data.audit_events.push({
      ...
      customer_id: license.customer_id,
      metadata: { extension_days: extensionDays },
      created_at: license.updated_at,
    });
    writeData(data);
    return sendJson(response, 200, { license: safeLicense(license) });
  }
  const licenseReissueMatch = pathname.match(
    /^\/api\/licenses\/([^/]+)\/reissue$/,
  );
  ...
  if (request.method === "DELETE" && customerDeleteMatch) {
    ...
    if (
      data.licenses.some(
        (license) =>
          license.customer_id === customerId && license.status !== "revoked",
      )
    )
      return sendError(
        response,
        409,
        "Customer cannot be deleted while active licenses reference it",
      );
  ...
  if (request.method === "POST" && pathname === "/api/customers") {
    ...
    if (!name || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
      return sendError(
        response,
        400,
        "A customer name and valid email are required",
      );
    const data = readData();
    if (
      data.customers.some((customer) => customer.email.toLowerCase() === email)
    )
      return sendError(
        response,
        409,
  ...
  if (request.method === "POST" && pathname === "/api/app-profiles") {
    ...
    const name = String(body.name || "").trim();
    const plans =
      Array.isArray(body.plans) && body.plans.length
        ? body.plans.map((plan) => String(plan).toLowerCase())
        : ["base", "pro"];
    const features = Array.isArray(body.features)
      ? body.features.map(String)
      : [];
    const installationLimit = Number(body.installation_limit || 1);
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(appId) || !name || profiles[appId])
      return sendError(
        response,
        400,
        "Profile ID must be unique, lowercase, and use hyphens only",
      );
    if (!Number.isInteger(installationLimit) || installationLimit < 1)
      return sendError(
        response,
        400,
        "Installation limit must be a positive whole number",
      );
    const profile = {
      id: id("app_profile"),
      app_id: appId,
  ...
  if (request.method === "PUT" && profileUpdateMatch) {
    ...
    if (!profile)
      return sendError(response, 404, "Application profile not found");
    const body = await readBody(request);
    const name = String(body.name || profile.name).trim();
    const installationLimit = Number(
      body.installation_limit || profile.installation_limit,
    );
    const features = Array.isArray(body.features)
      ? body.features.map(String)
      : profile.features;
    if (!name || !Number.isInteger(installationLimit) || installationLimit < 1)
      return sendError(
        response,
        400,
        "Profile name and installation limit are required",
      );
    const data = readData();
    ...
    data.app_profiles = data.app_profiles.map((record) =>
      record.app_id === appId ? profile : record,
    );
    data.audit_events.push({
      id: id("audit"),
      actor: "admin",
      action: "app_profile.updated",
      app_profile_id: profile.id,
      created_at: new Date().toISOString(),
    });
    writeData(data);
  ...
  const profileOptionMatch = pathname.match(
    /^\/api\/app-profiles\/([^/]+)\/license-options(?:\/([^/]+))?$/,
  );
  if (
    profileOptionMatch &&
    request.method === "POST" &&
    !profileOptionMatch[2]
  ) {
    ...
    if (
      ...
      return sendError(
        response,
        400,
        "Option type, label, and a positive whole-day length are required",
      );
    normalizeProfile(profile);
    if (
      profile.license_options.some(
        (option) => option.type === type && option.length_days === lengthDays,
      )
    )
      return sendError(response, 409, "This license option already exists");
    const option = {
      id: `${type}-${lengthDays}-${crypto.randomBytes(3).toString("hex")}`,
      type,
      label,
  ...
  if (
    ...
    normalizeProfile(profile);
    const data = readData();
    if (!profile.license_options.some((option) => option.id === optionId))
      return sendError(response, 404, "License option not found");
    if (
      data.licenses.some(
        (license) =>
          license.app_id === appId &&
          license.option_id === optionId &&
          license.status !== "revoked",
      )
    )
      return sendError(
        response,
        409,
        "License option cannot be removed while active licenses reference it",
      );
    profile.license_options = profile.license_options.filter(
  ...
  if (request.method === "DELETE" && profileDeleteMatch) {
    ...
    if (
      data.licenses.some(
        (license) => license.app_id === appId && license.status !== "revoked",
      )
    )
      return sendError(
        response,
        409,
        "Profile cannot be deleted while active licenses reference it",
      );
    delete profiles[appId];
    data.app_profiles = data.app_profiles.filter(
      (profile) => profile.app_id !== appId,
    );
    data.audit_events.push({
      id: id("audit"),
      actor: "admin",
  ...
  if (request.method === "POST" && pathname === "/api/licenses/issue") {
    ...
    const option =
      profile &&
      (normalizeProfile(profile).license_options.find(
        (candidate) => candidate.id === body.option_id,
      ) ||
        normalizeProfile(profile).license_options.find(
          (candidate) =>
            candidate.type === body.license_type &&
            (candidate.type === "pro" ? body.plan === "pro" : true),
        ));
    if (!profile || !option)
      return sendError(
        response,
        400,
        "Application profile or license option is invalid",
      );
    const data = readData();
    const selectedCustomer = body.customer_id
      ? data.customers.find((customer) => customer.id === body.customer_id)
      : null;
    if (body.customer_id && !selectedCustomer)
    ...
    const payload = {
      ...
      option_length_days: option.length_days,
      plan: option.type,
      license_type: option.type,
      features: option.type === "pro" ? profile.features : [],
      issued_at: now.toISOString(),
      expires_at: expires.toISOString(),
      installation_limit: profile.installation_limit,
      key_id: "issuer-ed25519-01",
    };
    const token = signLicense(payload);
  ...
  if (request.method === "POST" && pathname === "/api/licenses/verify") {
    ...
    try {
      return sendJson(response, 200, {
        valid: true,
        claims: verifyLicenseToken(body.token),
      });
    } catch (error) {
      return sendError(response, 400, error.message);
    }
...
server.listen(port, host, () =>
  console.log(`Keystone portal running at http://${host}:${port}`),
);
